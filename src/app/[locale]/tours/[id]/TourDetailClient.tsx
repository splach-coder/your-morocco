...
// Import the type if available, otherwise define loose type
interface TourDetailClientProps {
    tour: any; // We'll rely on the data passed from the server component
    relatedTours: any[];
    locale: string;
}

export default function TourDetailClient({ tour, relatedTours, locale }: TourDetailClientProps) {
    const t = useTranslations('DetailPage');
    const { setBookingItem, getWhatsAppMessage } = useBooking();

    // Safety check - should be handled by parent but good to have
    if (!tour) return null;
    ...
    useEffect(() => {
        if (tour.id && tour.title) {
            setBookingItem(tour.id, 'tour', tour.title, pricing);
        }
    }, [tour.id, tour.title]);
                        ...
                        {tour.suitable_for && (
                            <section>
                                ...
                                <div className="flex flex-wrap gap-3">
                                    {tour.suitable_for.map((item: string, index: number) => (
